

package src.graphics.kerai_src;

import com.badlogic.gdx.graphics.g3d.Model;
import com.badlogic.gdx.utils.Array;

/**
 * A NodeAnimation defines keyframes for a {@link Node2} in a {@link Model}. The keyframes
 * are given as a translation vector, a rotation quaternion and a scale vector. Keyframes are 
 * interpolated linearly for now. Keytimes are given in seconds.
 * @author badlogic
 *
 */
public class NodeAnimation2 {
    /** the Node affected by this animation **/
    public Node2 node;
    /** the keyframes, sorted by time, ascending **/
    public NodeKeyframe2 keyframes[];
    //public Array<NodeKeyframe2> keyframes = new Array<NodeKeyframe2>();
}